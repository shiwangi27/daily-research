...
// 2-link IK: mid-joint (knee/elbow) world point for a limb root P -> target F.
// pick "x" bends forward (max x), "y" bends upward (max y). Enables foot/hand planting.
function ik2(P,F,L1,L2,pick){let vx=F[0]-P[0],vy=F[1]-P[1],dd=Math.hypot(vx,vy)||1e-3;
  const mx=L1+L2-1e-3,mn=Math.abs(L1-L2)+1e-3;
  if(dd>mx){vx*=mx/dd;vy*=mx/dd;dd=mx;}else if(dd<mn){vx*=mn/dd;vy*=mn/dd;dd=mn;}
  F=[P[0]+vx,P[1]+vy];
  const t=(L1*L1-L2*L2+dd*dd)/(2*dd),h=Math.sqrt(Math.max(0,L1*L1-t*t));
  const m=[P[0]+t*vx/dd,P[1]+t*vy/dd],ox=-vy/dd*h,oy=vx/dd*h;
  const k1=[m[0]+ox,m[1]+oy],k2=[m[0]-ox,m[1]-oy];
  const sel=pick==="y"?(k1[1]>=k2[1]):(k1[0]>=k2[0]);
  return {mid:sel?k1:k2,end:F};}
// a foot/hand contact over a gait cycle: planted on the floor and sliding BACK during stance,
// then arcing forward through the air during swing.
function contact(x0,phase,S,lift){let u=((phase/TAU)%1+1)%1,x,y;
  if(u<0.5){const f=u/0.5;x=S*(0.5-f);y=0;}                     // stance: front -> back, on the floor
  else{const f=(u-0.5)/0.5;x=S*(f-0.5);y=lift*Math.sin(Math.PI*f);}  // swing: back -> front, lifted
  return [x0+x,y];}

function walkRig(a,p){const S=0.52,lift=0.28,ph=TAU*p,py=a.py-0.09+0.03*Math.sin(2*TAU*p);
  const rg={px:a.px,py:py,torso:a.torso+0.05*Math.sin(2*TAU*p),head:a.head,legs:[],arms:[]};
  [1,0].forEach(near=>{const o=near?0:1;
    const f=contact(a.px,ph+(o?Math.PI:0),S,lift),ik=ik2([rg.px,rg.py],f,L.thigh,L.shin,"x");
    rg.legs.push({ik:1,kneeW:ik.mid,footW:ik.end,far:!!o});
...
function rigOf(a,key,p){
  if(key==="walk")return walkRig(a,p);
  if(key==="crawl")return crawlRig(a,p);
  let base=Object.assign({},a),bat=0;
  const e=0.5-0.5*Math.cos(TAU*p), sw=Math.sin(TAU*p);
  if(key==="head")base.head=lerp(0.35,a.head,e);
  else if(key==="reach")bat=1;
  ...
  [1,0].forEach(near=>{const o=near?0:1;
    let sh=base.shoulder;
    if(bat)sh=base.shoulder+(o?-0.18:0.18)*sw; else sh=base.shoulder+(o?0.14:-0.04);
    rg.legs.push({hip:base.hip+(o?-0.13:0.05),knee:base.knee,far:!!o});
    rg.arms.push({shoulder:sh,elbow:base.elbow,far:!!o});});
  return {rig:rg,scroll:0};
}
...
function rigPoints(rg,fit){const T=fit.T,b=body(rg);
  return {pelvis:T(b.pelvis),chest:T(b.chest),neck:T(b.neck),head:T(b.head),
    legs:rg.legs.map(l=>l.ik?{knee:T(l.kneeW),foot:T(l.footW),far:l.far}
      :(function(){const q=legPts(b.pelvis,l.hip,l.knee);return {knee:T(q.knee),foot:T(q.foot),far:l.far};})()),
    arms:rg.arms.map(m=>m.ik?{elbow:T(m.elbowW),hand:T(m.handW),far:m.far}
      :(function(){const q=armPts(b.chest,m.shoulder,m.elbow);return {elbow:T(q.elbow),hand:T(q.hand),far:m.far};})()),
...
function drawBaby(rg,fit,opts){opts=opts||{};const P=rigPoints(rg,fit),s=fit.s;P._s=s;
  ...
  // supine: reflect (x,y)->(-y,-x) so the face points UP and the hair sits on the crown
  // (far from the neck); other poses just rotate.
  if(opts.supine)ctx.transform(0,-1,-1,0,0,0); else ctx.rotate(opts.face||0);
...
const supineFor=k=>k==="reach";  // lie on the back -> reflect the face to look up
...
function loop(){cancelAnimationFrame(raf);t0=null;const m=M[cur];
  ...
  if(reduce){paint(staticRig(m.angles),{rattle:m.key==="reach",supine:supineFor(m.key)});return;}
  const per=2600;(function fr(ts){if(t0===null)t0=ts;const p=((ts-t0)%per)/per;const r=rigOf(m.angles,m.key,p);
    advScroll(r.scroll);paint(r.rig,{rattle:m.key==="reach",supine:supineFor(m.key)});raf=requestAnimationFrame(fr);})(0);}
...
function playAll(){cur=-1;cancelAnimationFrame(raf);t0=null;floorX=0;
  ...
  (function fr(ts){if(t0===null)t0=ts;let e=(ts-t0)%total,i=0;while(e>segs[i].dur){e-=segs[i].dur;i++;}
    ...
    else{const r=rigOf(m.angles,m.key,p);advScroll(r.scroll);paint(r.rig,{rattle:m.key==="reach",supine:supineFor(m.key)});
      setInfo(m.week,m.title,{t:m.success?"Mastered":"Learning",c:m.success?"master":"learn"},stars(m),m.blurb);
...
function scrubTo(w){cur=-1;cancelAnimationFrame(raf);t0=null;floorX=0;
  ...
  const per=2900;(function fr(ts){if(t0===null)t0=ts;const sway=0.05*Math.sin(TAU*((ts-t0)%per)/per);
    ...
    paint(staticRig(a),{rattle:ns.key==="reach"&&on,supine:on&&supineFor(ns.key)});
